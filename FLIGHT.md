...
```
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add ~/dev/mjbraun/chiron/bin/chiron-server
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp ~/dev/mjbraun/chiron/bin/chiron-server
```
...
```
cd ~/dev/mjbraun/chiron/server && .venv/bin/python usb_bridge.py
```
