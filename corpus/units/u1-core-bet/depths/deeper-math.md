...
Therefore, with $\mathcal{L}$ in nats - $\mathcal{L}$ being the training
loss, the corpus average of $-\ln$ of the probability the model assigned to
each symbol that actually occurred; the objective section below states it
exactly:
