...
owns as a PDF or an EPUB is a source too: copy it to the sprite, add an
index entry with
`fetch: {kind: file, path: /home/sprite/chiron/sources/<name>.epub}`
and the licence it was read under, and name it with `-source`. An EPUB
is split at its spine, one chapter per document, which is where its own
reader turns the page; a PDF at the pages that open with a chapter
heading. To see
