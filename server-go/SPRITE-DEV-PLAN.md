...
**Phase A: the tunnel and the Mac.**
...
Still open:
1. Served instance as checkout, or deploy target. Recommended: deploy
   target, because a half-edited working tree must not be what the reader
   is served, and `make deploy` makes the extra step a one-liner.
2. Who answers on port 8080. Recommended: a tiny `chiron-gate` that owns
   8080, serves `/ssh`, and reverse-proxies everything else to the book
   server on 8081. Reason: with the tunnel inside the book binary, a bad
   deploy takes down the book *and* the only way in, and rescue then needs
   the personal-account CLI, which is exactly the situation to design out.
   The gate changes rarely and carries no book logic. Alternative: one
   binary, `/ssh` inside the book server, accepting that lockout risk.
3. Bootstrap timing: Phase A needs one personal-account `sprite console`
   session for `sshd` and the first key. Matt runs it; everything after
   goes through `ssh chiron`.
